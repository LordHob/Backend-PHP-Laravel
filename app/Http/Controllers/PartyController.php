...
class PartyController extends Controller
{
    ...
    public function joinParty(Request $request)
    {
        ...
        try {
            ...
            $joinParty = Membership::where('userId', '=', $userId)->where('partyId', '=', $partyId)->get();
    ...
    public function getMyParties()
    {
        ...
        try {
            ...
            return Party::selectRaw('memberships.partyId, parties.name, users.id as userId, users.userName')
                ->join('memberships', 'memberships.partyId', '=', 'parties.id')
    ...
    public function getPartyMembers(Request $request)
    {
        ...
        try {
            ...
            $party = Party::selectRaw('memberships.partyId, parties.name, users.id, users.userName')
                ->join('memberships', 'memberships.partyId', '=', 'parties.id')
                ->join('users', 'users.id', '=', 'memberships.userId')
    ...
    public function quitParty(Request $request)
    {
        ...
        try {
            ...
            return Membership::where('userId', '=', $userId)->where('partyId', '=', $partyId)->delete($userId);
        } catch (QueryException $error) {
    ...
    public function kickFromParty(Request $request)
    {
        ...
        try {
            ...
            return Membership::selectRaw('memberships.userId, memberships.partyId')
                ->join('parties', 'parties.id', '=', 'memberships.partyId')
                ->join('users', 'users.id', '=', 'memberships.userId')
                ->where('parties.owner', '=', $userId)
                ->where('parties.Id', '=', $partyId)
                ->where('memberships.userId', '=', $userToKick)
                ->delete($userToKick);
        } catch (QueryException $error) {
